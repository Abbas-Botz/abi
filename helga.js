...
try {
		...
		const katalog = (teks) => {
             res = helga.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Abbas Botz_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             helga.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = helga.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '628995820020-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "*_© Abbas Botz_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			"groupJid": "628995820020-1616169743@g.us",
...
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database Abbas Botz```'
...
⚠️ Segala kebijakan dan ketentuan AbbasBotz di pegang oleh owner dan segala perubahan kebijakan, sewaktu waktu owner berhak mencabut, memblokir user(*﹏*) 

Arigatou Gozaimasu! Untuk kalian user ramah dan Beberapa orang yg ikut membantu juga dalam project pembuatan AbbasBotz-
😖🙏

For GitHub : https://github.com/Abbas-Botz
YouTube Helga : https://youtube.com/c/AbbasMods
`
sendButLocation(from, `${menu}`, "*Abbas Botz*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
break
     case 'allmenu':
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
        ptod = "628995820020@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
...
Giliran = @${tty.player1.split('@')[0]}`
 helga.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     helga.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Subscribe Channel Abbas Botz*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Abbas Botz",body:"",previewType:"Helga Base",thumbnail:fs.readFileSync('./helga.jpg'),sourceUrl:"https://wa.me/628995820020"}}})
	}
    }
    }
    }
